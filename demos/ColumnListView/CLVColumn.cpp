...
CLVColumn::CLVColumn(const char* label,float width,uint32 flags,float min_width)
{
	if(flags & CLV_EXPANDER)
	{
		...
		width = 20.0;
		min_width = 20.0;
		flags &= CLV_NOT_MOVABLE | CLV_LOCK_AT_BEGINNING | CLV_NOT_SHOWN | CLV_LOCK_WITH_RIGHT;
...
void CLVColumn::SetShown(bool Shown)
{
	...
	{
		if(Shown)
			fFlags &= 0xFFFFFFFF^CLV_NOT_SHOWN;
		else
			fFlags |= CLV_NOT_SHOWN;
		if(fParent)
		{
			...
			if(fFlags & CLV_EXPANDER)
			{
				if(!Shown)
					...
					fParent->fExpanderColumn = fParent->fColumnList.IndexOf(this);
			}
			if(ParentWindow)
				ParentWindow->Unlock();
		}
