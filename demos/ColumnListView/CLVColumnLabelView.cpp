...
{
	int16 NumberOfGroups = fDragGroups.CountItems();
	for(int16 Counter = 0; Counter < NumberOfGroups; Counter++)
...
void CLVColumnLabelView::Draw(BRect UpdateRect)
{
	...
	int16 NumberOfColumns = fDisplayList->CountItems();
	BPoint Start,Stop;
	for(int16 ColumnDraw = 0; ColumnDraw < NumberOfColumns; ColumnDraw++)
	{
		...
		if(ThisColumn->IsShown())
		{
			...
			{
				for(int16 ColumnCounter = ColumnDraw+1; ColumnCounter < NumberOfColumns;
					ColumnCounter++)
				{
				...
				ViewBounds.bottom)))
			{
				...
				if(ThisColumn->fLabel)
				{
					...
					//Underline if this is the selected sort column
					if((ThisColumn == fParent->fSortKey && fParent->fSortMode != NoSort) ||
						(ThisColumn == fParent->fBoolGroupKey && fParent->fBoolGroupMode != NoGroup))
					{
					...
					if(MousePos.x<fPreviousMousePos.x || MousePos.x>fPreviousMousePos.x)
					{
						...
						do
						{
							...
							if(ColumnSnapped)
							{
								...
								{
									int16 NumberOfColumns = fDisplayList->CountItems();
									for(int16 ColumnCounter = fDisplayList->IndexOf(LastSwapColumn)+1;
		...
		else if(Buttons == 0)
		{
			...
			if(!fColumnDragging && !fColumnResizing)
			{
				...
				if(ColumnFlags&CLV_SORT_KEYABLE)
				{
					...
					if(fParent->fSortKey == fColumnClicked)
					...
					else
					{
						...
						if(fParent->Sorting(NULL) == NoSort)
							fParent->SetSortMode(Ascending);
					}
				}
				else if(ColumnFlags&CLV_BOOL_GROUPABLE)
				{
					//The column is a "groupable" column
					if(fParent->fBoolGroupKey == fColumnClicked)
						//The column was already selected; switch group modes
						fParent->ReverseBoolGroupMode();
					else
					{
						//The user selected this column for grouping
						fParent->SetBoolGroupKey(fParent->fColumnList.IndexOf(fColumnClicked));
						if(fParent->BoolGrouping(NULL) == NoGroup)
							fParent->SetBoolGroupMode(TrueFirst);
					}
				}
			}
...
void CLVColumnLabelView::ShiftDragGroup(int16 NewPos)
//Shift the drag group into a new position
{
	int16 NumberOfGroups = fDragGroups.CountItems();
	int16 GroupCounter;
	CLVDragGroup* ThisGroup;
	int16 NumberOfColumnsInGroup;
	int16 ColumnCounter;
	...
	*fDisplayList = NewDisplayList;
	...
	int16* NewOrder = fParent->DisplayOrder();
